...
(STRUMPACKKernel kernel, scalar_t* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
  ...
  opts.set_p(oversampling);
  opts.set_approximate_neighbors(vann);
  opts.set_rel_tol((double)(pow(10.0,epow)));
...
(STRUMPACKKernel kernel, scalar_t* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
  ...
  STRUMPACK_kernel_fit_HSS_MPI<scalar_t>(kernel, labels, oversampling, vann, epow, argc, argv);
#endif
}
#endif
...
extern "C" {
  ...
  (STRUMPACKKernel kernel, double* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
    STRUMPACK_kernel_fit_HSS<double>(kernel, labels, oversampling, vann, epow, argc, argv);
  }
  void STRUMPACK_kernel_fit_HSS_float
  (STRUMPACKKernel kernel, float* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
    STRUMPACK_kernel_fit_HSS<float>(kernel, labels, oversampling, vann, epow, argc, argv);
  }

#if defined(STRUMPACK_USE_MPI)
  void STRUMPACK_kernel_fit_HSS_MPI_double
  (STRUMPACKKernel kernel, double* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
    STRUMPACK_kernel_fit_HSS_MPI<double>(kernel, labels, oversampling, vann, epow, argc, argv);
  }
  void STRUMPACK_kernel_fit_HSS_MPI_float
  (STRUMPACKKernel kernel, float* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
    STRUMPACK_kernel_fit_HSS_MPI<float>(kernel, labels, oversampling, vann, epow, argc, argv);
  }

  void STRUMPACK_kernel_fit_HODLR_MPI_double
  (STRUMPACKKernel kernel, double* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
    STRUMPACK_kernel_fit_HODLR_MPI<double>(kernel, labels, oversampling, vann, epow, argc, argv);
  }
  void STRUMPACK_kernel_fit_HODLR_MPI_float
  (STRUMPACKKernel kernel, float* labels, int oversampling, int vann, int epow, int argc, char* argv[]) {
    STRUMPACK_kernel_fit_HODLR_MPI<float>(kernel, labels, oversampling, vann, epow, argc, argv);
  }
#endif

...
def objectives(point):
    ...
    comm = MPI.COMM_SELF.Spawn("%s/KernelRegressionMPI.py"%(RUNDIR), args=['%s/%s'%(INPUTDIR,datafile), '%s'%(h),'%s'%(Lambda),'%s'%(degree), '%s'%(p), '%s'%(vann), '%s'%(epow)], maxprocs=nproc,info=info)
...
def main():
    ...
    p = Integer(1, 50, transform="normalize", name="p") # oversampling parameter
    vann = Integer(1, 256, transform="normalize", name="vann") # number of approximate nearest neighbors
    epow = Integer(-7, -1, transform="normalize", name="epow") # relative compression tolerance

...
def objectives(point):                  # should always use this name for user-defined objective function
	...
	params = ['ROWPERM', ROWPERM, 'COLPERM', COLPERM, 'NSUP', NSUP, 'NREL', NREL, 'nbx', nbx, 'nby', nby]
	...
	print('exec', "./nimrod_spawn", 'nproc', nprocmax, 'env', 'OMP_NUM_THREADS=%d' %(nthreads), 'NSUP=%d' %(NSUP), 'NREL=%d' %(NREL))
	comm = MPI.COMM_SELF.Spawn("./nimrod_spawn", maxprocs=nprocmax,info=info)
	...
	comm.Disconnect()	
	retval = tmpdata[0]
	print(params, ' nimrod time: ', retval)
...
def main():
	...
	for tid in range(NI):
		print("tid: %d"%(tid))
		print("    mx:%s my:%s lphi:%s"%(data.I[tid][0],data.I[tid][2],data.I[tid][2]))
		print("    Ps ", data.P[tid])
		print("    Os ", data.O[tid])

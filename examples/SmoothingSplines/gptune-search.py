...
def objectives(point):
    ...
    with open("gptune.db/gptune_search."+str(nrun)+".log", "a") as f_out:
...
def main():
    ...
    k = Integer(4, 11, transform="normalize", name="k")
    ...
    if not os.path.exists("gptune.db"):
        os.system("mkdir -p gptune.db")
    with open("gptune.db/gptune_search."+str(nrun)+".log", "w") as f_out:
        f_out.write("NKnots,Lambda,RegressionTime,InTestTime,InMSE,InR2,InAR2,OutTestTime,OutMSE,OutR2,OutAR2\n")

    dataset = "gptune-demo-"+str(giventask[0][0])+"-10000000"

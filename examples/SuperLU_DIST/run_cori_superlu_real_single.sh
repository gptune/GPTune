...
tuner='GPTune'
mpirun --oversubscribe --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./superlu_single.py  -nodes ${nodes} -cores ${cores} -nprocmin_pernode ${nprocmin_pernode} -ntask 1 -nrun ${nrun} -machine cori -optimization ${tuner} | tee a.out_superlu_MLA_SO_nodes${nodes}_cores${cores}_nprocmin_pernode${nprocmin_pernode}_nrun${nrun}_${tuner}

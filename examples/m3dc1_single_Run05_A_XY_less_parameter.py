...
def objectives(point):                  # should always use this name for user-defined objective function
	...
	# fixed number of jacobi blocks that equals number of planes
	for line in fin:
		if(line.find("-pc_bjacobi_blocks")!=-1):
			m = re.search('pc_bjacobi_blocks (\d+)', line, re.IGNORECASE)
			nblock = int(m.group(1))
	fin.close()
			
	COLPERM = 'METIS_AT_PLUS_A'
	ROWPERM = point['ROWPERM']
	# nblock = 32  
	nproc = int(nprocmax/nblock)
...
def main():
	...
	IS = Space([example])
	PS = Space([ROWPERM, nprows, NSUP, NREL])

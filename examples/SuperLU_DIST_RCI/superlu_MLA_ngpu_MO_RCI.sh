...
python ./superlu_MLA_ngpu_MO_RCI.py -nrun $nrun -npernode $npernode
...
arry=($(grep 'Sum-of-all' a.out | grep -Eo '[+-]?[0-9]+([.][0-9]+)?'))  # consider summed peak memory instead of factor memory
result2=${arry[0]}

...
order=0
tuner=GPTune
$RUN python ./butterflypack_RFcavity_multimode_HO.py -order ${order} -nthreads ${nthreads} -ntask ${ntask} -nrun ${nrun} -optimization $tuner | tee a.out_butterflypackRFcavity_nodes${nodes}_cores${cores}_ntask${ntask}_nrun${nrun}_order${order}_${tuner}
# tuner=SIMPLEX
# $RUN python ./butterflypack_RFcavity_multimode_HO_simplex.py -order ${order} -nthreads ${nthreads} -ntask ${ntask} -nrun ${nrun} -optimization $tuner | tee a.out_butterflypackRFcavity_nodes${nodes}_cores${cores}_ntask${ntask}_nrun${nrun}_order${order}_${tuner}

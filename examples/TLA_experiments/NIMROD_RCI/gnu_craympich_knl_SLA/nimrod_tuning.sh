...
database="gptune.db/NIMROD_GNU_CrayMPICH_KNL_SLA.json"  # the phrase SuperLU_DIST should match the application name defined in .gptune/meta.jason
...
python ./nimrod_tuning.py -nstep $nstep -expid $expid -seed $seed -nrun ${nrun} -npilot ${npilot}
...
for repeat in {1,2,3}
do
    if [[ ${result_arr[${repeat}-1]} == 0.0 ]]; then
        echo there is faulty execution at repeat${repeat}
        result_arr[${repeat}-1]=1e30
        result=1e30
    fi
done

...
if __name__ == "__main__":
    ...
    experiment_name = "hypre_tuning_space_reduction"
    ...
    for plot_num in [0,1,2,3]:
        ...
        for tuner in ["tuning_all", "tuning_reduced"]:
            batches_num_func_eval = []
            batches_best_tuning_result = []

            for batch_num in [0,1,2,3,4]:
                if tuner == "tuning_all":
                    search_logfile = "gptune.db/Hypre-Full-"+str(batch_num)+".json"
                elif tuner == "tuning_reduced":
                    search_logfile = "gptune.db/Hypre-Reduced-"+str(batch_num)+".json"
                ...
                with open(search_logfile, "r") as f_in:
                    ...
                    for i in range(0, len(function_evaluations), 1):
                        ...
                        runtime = func_eval["evaluation_result"]["r"]
            ...
            if len(batches_num_func_eval) >= 1:
                ...
                if tuner == "tuning_all":
                    ...
                    label_name = "Original tuning problem (12 parameters)"
                    if plot_num == 3:
                        label_name = "Original tuning problem (12 parameters), 10th result: " + str(tuning_all_10th) + "s, 20th result: " + str(tuning_all_20th)+"s"
        ...
        if plot_num == 2:
            ...
            ax.set_yticklabels([1.0,1.5,2.0,2.5], fontsize=16)

        ax.tick_params(labelsize=16)
        ax.grid(axis='y')

    #fig.suptitle("Tuning on SuperLU_DIST (matrix: " + target + ")", fontsize=20)
    fig.suptitle("Tuning on Hypre (nx=ny=nz=120)", fontsize=20)

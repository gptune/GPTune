...
#########################################
    ...
    while True:
        ...
        Nproc = np.random.randint(1, 32)
        Pz = int(Nproc / (Px*Py))
        strong_threshold = 0.25 #np.random.rand()
        trunc_factor = 0.0 #np.random.rand()
        P_max_elmts = 4 #np.random.randint(1, 12)
        coarsen_type = '10' #str(np.random.choice(['0', '1', '2', '3', '4', '6', '8', '10']))
        relax_type = '-1' #str(np.random.choice(['-1', '0', '6', '8', '16', '18']))
        interp_type = '6' #str(np.random.choice(['0', '3', '4', '5', '6', '8', '12']))

        if Px * Py <= Nproc:
            if not(coarsen_type=='0' and P_max_elmts==10 and relax_type=='18' and smooth_type=='6' and smooth_num_levels==3 and interp_type=='8' and agg_num_levels==1):
                break
    ...
    return runtime, { "Px":int(Px), "Py":int(Py), "Nproc":int(Nproc), "strong_threshold":strong_threshold, "trunc_factor":trunc_factor, "P_max_elmts":int(P_max_elmts), "coarsen_type":coarsen_type, "relax_type":relax_type, "smooth_type":smooth_type, "smooth_num_levels":int(smooth_num_levels), "interp_type":interp_type, "agg_num_levels":int(agg_num_levels) }
...
def main(): 
    ...
    tuning_metadata = {
        "tuning_problem_name": "Hypre-Reduced-"+str(nbatch),
        "use_crowd_repo": "no",
        "no_load_check": "yes",
    ...
    PS = Space([smooth_type, smooth_num_levels, agg_num_levels])
    OS = Space([r])

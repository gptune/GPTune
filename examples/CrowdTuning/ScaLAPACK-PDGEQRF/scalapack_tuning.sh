...
do
   case $opt in
      a ) tuning_method=$OPTARG ;;
      b ) nrun=$OPTARG ;;
      c ) npilot=$OPTARG ;;
      d ) nbatch=$OPTARG ;;
      e ) nprocmin_pernode=$OPTARG ;;
      f ) input_m=$OPTARG ;;
      g ) input_n=$OPTARG ;;
...
database="gptune.db/PDGEQRF_${tuning_method}_${nbatch}.json"  # the phrase PDGEQRF should match the application name defined in .gptune/meta.jason
# rm -rf $database
...
python ./scalapack_tuning.py -tuning_method $tuning_method -nbatch $nbatch -nrun $nrun -npilot $npilot -input_m $input_m -input_n $input_n -bunit $bunit -nprocmin_pernode $nprocmin_pernode
...
jq --arg v0 $obj --argjson v1 $idx --argjson v2 [$result] '.func_eval[$v1].evaluation_detail[$v0].evaluations=$v2' $database > tmp.json && mv tmp.json $database

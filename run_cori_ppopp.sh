...
if [ $ex = 'test' ];then
    ...
    $RUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./demo.py 
    cd $GPTUNEROOT/examples/Scalapack-PDGEQRF
    rm -rf gptune.db/*.json
    $RUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./scalapack_MLA.py -mmax 1300 -nmax 1300 -nodes 1 -cores 4 -ntask 2 -nrun 10 -machine mymachine -jobid 0
    cd $GPTUNEROOT/examples/SuperLU_DIST
    rm -rf gptune.db/*.json
    $RUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./superlu_MLA.py  -nodes 1 -cores 4 -ntask 1 -nrun 20 -machine mymachine
...
# this example performs one MLA iteration using $\epsilon=80$ and $\delta=20$ samples of the analytical function (see Table 2). Suppose your machine has 1 node with 16 cores, run the following two configurations and compare 'time_search':xxx and 'time_model':xxx from the runlogs.    
    cd $GPTUNEROOT/examples/GPTune-Demo
    rm -rf gptune.db/*.json
    $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./demo_parallelperformance.py -ntask 20 -nrun 80  | tee a.out_seqential # this is the sequential benchmark
    rm -rf gptune.db/*.json
    $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./demo_parallelperformance.py -machine mymachine -nodes 1 -cores 16 -ntask 20 -nrun 80 -distparallel 1 | tee a.out_parallel # this is parallel modeling and search
elif [ $ex = 'Fig.4' ];then
    cd $GPTUNEROOT/examples/postprocess/demo/
    bash parse_plot.sh
...
# This example autotunes the runtime of SCALPACK PDGEQRF on a 2000x2000 matrix with $\epsilon=10$ and $\delta=1$. Suppose that your machine has 1 node with 4 cores, each PDGEQRF run will use at most 3 MPI ranks. Run the following configurations with and without a performance model, check the "Popt  [x, x, x] Oopt  x" for best tuning parameters and runtime. Note that for this small matrix with small core counts, the performance model may not necessarily improve the tuning performance.  
    cd $GPTUNEROOT/examples/Scalapack-PDGEQRF
    rm -rf gptune.db/*.json
    $MPIRUN --oversubscribe --allow-run-as-root -n 1  python ./scalapack_MLA_perfmodel.py -mmax 2000 -nmax 2000 -nodes 1 -cores 4 -nprocmin_pernode 2 -ntask 1 -nrun 10 -machine mymachine -perfmodel 0 | tee a.out_qr_perf0
    rm -rf gptune.db/*.json
    $MPIRUN --oversubscribe --allow-run-as-root -n 1  python ./scalapack_MLA_perfmodel.py -mmax 2000 -nmax 2000 -nodes 1 -cores 4 -nprocmin_pernode 2 -ntask 1 -nrun 10 -machine mymachine -perfmodel 1 | tee a.out_qr_perf1
...
# This example autotunes the runtime of SCALPACK PDGEQRF for $\delta=2$ randomly generated-sized matrices with m,n<=2000 with $\epsilon=10$. Suppose that your machine has 1 node with 4 cores, each PDGEQRF run will use at most 3 MPI ranks. Run the following configurations and check the "Popt  [x, x, x] Oopt  x" for best tuning parameters and runtime for the two matrices.
    cd $GPTUNEROOT/examples/Scalapack-PDGEQRF 
    rm -rf gptune.db/*.json
    $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./scalapack_MLA.py -mmax 2000 -nmax 2000 -nodes 1 -cores 4 -ntask 2 -nrun 20 -machine mymachine -optimization GPTune | tee a.out_qr_MLA
...
# This example autotunes the runtime of SCALPACK PDGEQRF for $\delta=2$ randomly generated-sized matrices with m,n<=2000 with $\epsilon=10$. Suppose that your machine has 1 node with 4 cores, each PDGEQRF run will use at most 3 MPI ranks. Run the following configurations using the three tuners by setting -optimization to 'GPTune', 'hpbandster' or 'opentuner', check the "Popt  [x, x, x] Oopt  x" for best tuning parameters and runtime for the two matrices. 
    ...
    do
        rm -rf gptune.db/*.json
        $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./scalapack_MLA.py -mmax 2000 -nmax 2000 -nodes 1 -cores 4 -ntask 2 -nrun 20 -machine mymachine -optimization ${tuner} | tee a.out_qr_${tuner}
    done
...
# this example autotunes the runtime or memory of the numerical factorization of superlu_dist using a small matrix "big.rua" with $\epsilon=10$. Suppose that your machine has 1 node with 4 cores, each superlu_dist run will use at most 3 MPI ranks. Run the following configurations with setting -obj to 'time' or 'memory', and -optimization to 'GPTune', 'hpbandster' or 'opentuner' 
    ...
    do
        rm -rf gptune.db/*.json
        $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python superlu_MLA.py -nodes 1 -cores 4 -ntask 1 -nrun 10 -obj time -optimization ${tuner} -machine mymachine | tee a.out_superlu_${tuner}
    done
...
# this example demonstrates the multi-objective (runtime and memory) tuning of the numerical factorization of superlu_dist using three small matrices "big.rua", "g4.rua", "g20.rua" using $\epsilon=10$. Suppose that your machine has 1 node with 4 cores, each superlu_dist run will use at most 3 MPI ranks. The Pareto optima for each matrix are shown in "Popts" and "Oopts" at the bottom of the runlog.   
    cd $GPTUNEROOT/examples/SuperLU_DIST
    rm -rf gptune.db/*.json
    $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./superlu_MLA_MO.py  -nodes 1 -cores 4 -ntask 3 -nrun 10 -machine mymachine | tee a.out_superlu_multiobj
...
# this example autotunes the runtime for solving the 3D Poisson equation discretized on a nx x ny x nz grid using hypre with $\epsilon=10$ samples. The grid size is randomly generated with nx,ny,nz<=40. Suppose that your machine has 1 node with 4 cores, each hypre run will use at most 3 MPI ranks. Run the following configurations with setting -optimization to 'GPTune', 'hpbandster' or 'opentuner' 
    ...
    do    
        rm -rf gptune.db/*.json
        $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./hypre.py  -nodes 1 -cores 4 -nprocmin_pernode 1 -ntask 1 -nrun 10 -nxmax 40 -nymax 40 -nzmax 40 -machine mymachine -optimization ${tuner} | tee a.out_hypre_${tuner} 
    done
fi
done

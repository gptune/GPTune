...
if [ $ex = 'test' ];then
    ...
    $RUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python ./superlu_single.py  -nodes 1 -cores 4 -ntask 1 -nrun 20 -machine mymachine
...
# this example autotunes the runtime or memory of the numerical factorization of superlu_dist using a small matrix "big.rua" with $\epsilon=10$. Suppose that your machine has 1 node with 4 cores, each superlu_dist run will use at most 3 MPI ranks. Run the following configurations with setting -obj to 'time' or 'memory', and -optimization to 'GPTune', 'hpbandster' or 'opentuner' 
    ...
    for tuner in GPTune hpbandster opentuner 
    do
        $MPIRUN --mca pmix_server_max_wait 3600 --mca pmix_base_exchange_timeout 3600 --mca orte_abort_timeout 3600 --mca plm_rsh_no_tree_spawn true -n 1 python superlu_single.py -nodes 1 -cores 4 -ntask 1 -nrun 10 -obj time -optimization ${tuner} -machine mymachine | tee a.out_superlu_${tuner}
    done
